...
export default function Game3() {
  ...
  useEffect(() => {
    ...
    // apply a page-level class so global background / body rules can match Game1 styling
    try { document.body.classList.add('page-game3') } catch (e) { /* ignore */ }
    ...
    return () => {
      ...
      try { document.body.classList.remove('page-game3') } catch (e) { /* ignore */ }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])
  ...
  function handleKey(e) {
    ...
    // When game ended, ignore all non-space keys
    ...
    if (mapUp) {
      e.preventDefault()
      if (!running && !endedRef.current) startLoop()
  ...
  function changeDirection(newDir) {
    // ignore direction changes when game has ended
    if (endedRef.current) return
